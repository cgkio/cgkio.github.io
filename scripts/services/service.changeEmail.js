angular.module('myApp.service.changeEmail', [
  'myApp.service.login',
  ...
  function ($rootScope, firebaseRef, $timeout, $q, loginService) {
    ...
    return function (opts) {
      ...
      authenticate().then(loadOldProfile).then(createNewAccount).then(copyProfile).then(removeOldProfile).then(removeOldLogin).then(function () {
        cb && cb(null);
      }, cb).catch(errorFn);
    };
  }
